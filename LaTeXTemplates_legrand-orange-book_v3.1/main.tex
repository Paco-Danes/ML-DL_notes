...
\subsection*{Scalar multiplication}\index{Sectioning!Subsections}

Scalar multiplication involves scaling a vector by a scalar, which is a real number. When we multiply a vector by a scalar, the vector's magnitude is multiplied 
by the scalar value, while its direction remains unchanged. Mathematically, scalar multiplication is performed by multiplying each component of the vector by the scalar. If $\textbf{v} = [v_{1}, v_{2}, v_{3}]$ 
